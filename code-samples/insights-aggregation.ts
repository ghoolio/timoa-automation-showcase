...
export function normalizeInsightRange(value: unknown): InsightRange {
  return typeof value === "string" &&
    insightRanges.includes(value as InsightRange)
...
export function appointmentPaymentMethod(
  ...
  if (
    appointment.paymentMethod === paymentMethods.payOnSite ||
    appointment.paymentStatus === paymentStatuses.payOnSite
  ) {
    return "pay_at_salon";
  }
...
export function aggregateSalonInsights(input: {
  ...
  const confirmedAppointments = input.appointments.filter(
    (item) => item.status === "SCHEDULED",
  ).length;
  const cancelledAppointments = input.appointments.filter(
    (item) => item.status === "CANCELLED",
  ).length;
  const knownValueAppointments = input.appointments.filter(
    (item) => item.service.priceCents !== null,
  );
  ...
  const onlinePaidRevenueCents = input.onlinePaidPayments
    .filter(
      (payment) =>
        ...
        payment.provider === paymentProviders.paypal,
    )
  ...
  return {
    bookingOverview: {
      ...
      paidAppointments: input.appointments.filter(
        (item) => item.paymentStatus === paymentStatuses.paid,
      ).length,
    ...
    revenueOverview: {
      ...
      averageBookingValueCents:
        knownValueAppointments.length > 0
          ? Math.round(
              knownValueAppointments.reduce(
                (total, item) => total + (item.service.priceCents ?? 0),
                0,
              ) / knownValueAppointments.length,
            )
          : null,
    ...
    topServices: [...serviceCounts.values()]
      ...
      .map((item) => ({
        ...
        averageDurationMinutes: Math.round(
          item.totalDurationMinutes / item.bookingCount,
        ),
      })),
    paymentMethodMix: [...paymentMethodCounts.entries()].map(
      ([method, count]) => ({
        method,
        count,
